...
resource "openstack_compute_instance_v2" "k8s_calico_rr_no_floating_ip" {
  name              = "${var.cluster_name}-k8s-calico-rr-nf-${var.calico_rr_ass[count.index]}"
  count             = length(var.calico_rr_azs)
  availability_zone = "${var.calico_rr_azs[count.index]}"
  ...
  metadata = {
    ...
    cluster_id       = var.calico_rr_cluster_ids[count.index]
    node_taints      = "calico-rr=true:NoSchedule"
    AS               = var.calico_rr_ass[count.index]
    RR               = var.calico_rr_ips[count.index]
    TOR              = var.calico_rr_tors[count.index]
  }
}
